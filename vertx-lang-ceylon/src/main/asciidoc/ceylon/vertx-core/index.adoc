...
The operations run concurrently, and are joined. The `Handler` attached to the returned future is
...
compositeFuture.all(ArrayList { f1,  f2,  f3 });

----

While the `all` composition _waits_ until all futures are completed (or one fails), the `any` composition
_waits_ for the first completed future. `link:../../ceylondoc/vertx-core//CompositeFuture.object.html#any(io.vertx.core.Future,%20io.vertx.core.Future)[CompositeFuture.any]` takes several futures arguments
...
`link:../../ceylondoc/vertx-core//dns/AddressResolverOptions.type.html#setNdots(int)[ndots]`.
