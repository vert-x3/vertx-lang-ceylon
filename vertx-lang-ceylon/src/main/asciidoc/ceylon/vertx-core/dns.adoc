...
[source,ceylon]
----
value client = vertx.createDnsClient(DnsClientOptions {
  port = 53;
  host = "10.0.0.1";
  queryTimeout = 10000;
});
...
[source,ceylon]
----
value client1 = vertx.createDnsClient();

// Just the same but with a different query timeout
value client2 = vertx.createDnsClient(DnsClientOptions {
  queryTimeout = 10000;
});
...
[source,ceylon]
----
value client = vertx.createDnsClient(53, "9.9.9.9");
