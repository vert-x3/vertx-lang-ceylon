...
client.request(\iGET, "some-uri", (HttpClientResponse response) => {
  print("Received response with status code ``response.statusCode()``");
}).end();

client.request(\iPOST, "foo-uri", (HttpClientResponse response) => {
  print("Received response with status code ``response.statusCode()``");
}).end("some-data");
...
value buffer = buffer.buffer().appendDouble(12.34).appendLong(432);
request.end(buffer);
...
value buffer = buffer.buffer().appendFloat(12.3).appendInt(321);
request.end(buffer);
...
request.setChunked(true);
value pump = pump.pump(file, request);
file.endHandler((Void v) => {
  request.end()
});
pump.start();
...
request.continueHandler((Void v) => {
...
value buffer = buffer.buffer().appendInt(123).appendFloat(1.23);
