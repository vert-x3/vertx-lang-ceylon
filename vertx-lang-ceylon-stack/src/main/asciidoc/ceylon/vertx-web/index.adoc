...
value route1 = router.route("/some/path/").handler((RoutingContext routingContext) => {
  ...
  routingContext.vertx().setTimer(5000, (Long tid) => {
    routingContext.next()
  });
});
...
value route = router.route(\iPOST, "/catalogue/products/:productype/:productid/");
...
value route = router.route().method(\iPOST);
...
value route = router.route(\iPOST, "/some/path/");
...
value route = router.route().method(\iPOST).method(\iPUT);
...
value route = router.route(\iPUT, "myapi/orders").consumes("application/json").produces("application/json");
...
value sessionHandler = sessionHandler.create(store);
...
value basicAuthHandler = basicAuthHandler.create(authProvider);
...
value redirectAuthHandler = redirectAuthHandler.create(authProvider);
...
router.route().handler(corsHandler.create("vertx\\.io").allowedMethod(\iGET));
...
value sockJSHandler = sockJSHandler.create(vertx, options);
...
value sockJSHandler = sockJSHandler.create(vertx);
...
sockJSHandler.bridge(options, (BridgeEvent be) => {
  if (be.type() == \iPUBLISH || be.type() == \iRECEIVE) {
...
sockJSHandler.bridge(options, (BridgeEvent be) => {
  if (be.type() == \iPUBLISH || be.type() == \iSEND) {
    // Add some headers
...
value authProvider = oAuth2Auth.create(vertx, \iAUTH_CODE, JsonObject {
  clientID = "CLIENT_ID";
