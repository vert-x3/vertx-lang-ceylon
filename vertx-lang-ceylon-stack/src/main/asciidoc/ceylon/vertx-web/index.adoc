...
----
shared import io.vertx.ceylon.web "3.3.1";
----
...
router.get("/my-pretty-notfound-handler").handler((RoutingContext ctx) {
  ctx.response().setStatusCode(404).end("NOT FOUND fancy html here!!!");
});

router.get().failureHandler((RoutingContext ctx) {
...
value authProvider = oAuth2Auth.create(vertx, auth_code, OAuth2ClientOptions {
  clientID = "CLIENT_ID";
...
value authProvider = oAuth2Auth.createKeycloak(vertx, auth_code, JsonObject());

// create a oauth2 handler on our domain: "http://localhost:8000"
value oauth2 = oAuth2AuthHandler.create(authProvider, "http://localhost:8000");
...
router.route("/protected/somepage").handler((RoutingContext rc) {
  ...
  rc.user().isAuthorised("account:manage-account", (Boolean|Throwable manageAccount) {
    if (manageAccount) {
